...
func attachIfaceToBridge(ovsDriver *ovsdb.OvsDriver, hostIfaceName string, contIfaceName string, vlanTag *uint, contNetnsPath string, ovnPortName string) error {
	...
	err := ovsDriver.CreatePort(hostIfaceName, "", vlanTagNum, contIfaceName, contNetnsPath)
	if err != nil {
		return err
	}
...
func CmdAdd(args *skel.CmdArgs) error {
	...
	ovsDriver, err := ovsdb.NewOvsDriverWithUnix(bridgeName)
	if err != nil {
		return err
	}
...
func getOvsPortForContIface(ovsDriver *ovsdb.OvsDriver, contIface string, contNetnsPath string) (string, bool, error) {
	// External IDs were set on the port during ADD call.
	return ovsDriver.GetOvsPortForContIface(contIface, contNetnsPath)
}

func removeOvsPort(ovsDriver *ovsdb.OvsDriver, portName string) error {
