...
func NewOvsDriverWithUnix(bridgeName string) (*OvsDriver, error) {
...
func (self *OvsDriver) CreatePort(intfName, intfType string, vlanTag uint, contIfaceName string, contNetnsPath string) error {
...
func (self *OvsDriver) GetOvsPortForContIface(contIface, contNetnsPath string) (string, bool, error) {
	seachMap := map[string]string{"contNetns": contNetnsPath, "contIface": contIface}
	ovsmap, err := libovsdb.NewOvsMap(seachMap)
	if err != nil {
		return "", false, err
	}
	...
	transactionResult, err := self.ovsClient.Transact("Open_vSwitch", selectOp)
	if err != nil {
		return "", false, err
	}
	...
	if len(operationResult.Rows) != 1 {
		return "", false, fmt.Errorf("error get more than one port")
	}

	return fmt.Sprintf("%v", operationResult.Rows[0]["name"]), true, nil
