...
import (
	...
	consumer "github.com/k8snetworkplumbingwg/ovs-cni/pkg/mirror-consumer"
	"github.com/k8snetworkplumbingwg/ovs-cni/pkg/plugin"
	"github.com/k8snetworkplumbingwg/ovs-cni/pkg/types"
)

const consumerVlanID = 100
const consumerIFNAME1 = "eth0"
const consumerIFNAME2 = "eth1"
const consumerOvsPortOwner = "ovs-cni.network.kubevirt.io"

var _ = Describe("CNI mirror-consumer 0.3.0", func() { consumerTestFunc("0.3.0") })
var _ = Describe("CNI mirror-consumer 0.3.1", func() { consumerTestFunc("0.3.1") })
var _ = Describe("CNI mirror-consumer 0.4.0", func() { consumerTestFunc("0.4.0") })
var _ = Describe("CNI mirror-consumer 1.0.0", func() { consumerTestFunc("1.0.0") })

var consumerTestFunc = func(version string) {
	BeforeEach(func() {
		output, err := exec.Command("ovs-vsctl", "add-br", consumerBridgeName).CombinedOutput()
		...
		Eventually(func() error {
			bridgeLink, err = netlink.LinkByName(consumerBridgeName)
			return err
	...
	AfterEach(func() {
		output, err := exec.Command("ovs-vsctl", "del-br", consumerBridgeName).CombinedOutput()
		Expect(err).NotTo(HaveOccurred(), "Failed to remove testing OVS bridge: %v", string(output[:]))
	})

	consumerCreateInterfaces := func(ifName string, targetNs ns.NetNS) *current.Result {
		confplugin := fmt.Sprintf(`{
			...
			"bridge": "%s",
			"vlan": %d
		}`, version, consumerBridgeName, consumerVlanID)
		args := &skel.CmdArgs{
	...
	consumerTestCheck := func(conf string, r cnitypes.Result, ifName string, targetNs ns.NetNS) {
		...
		err = cmdCheckWithArgs(args, func() error {
			return consumer.CmdCheck(args)
		})
		Expect(err).NotTo(HaveOccurred())
	}

	consumerTestDel := func(conf string, mirrors []types.Mirror, r cnitypes.Result, ifName string, targetNs ns.NetNS) {
	...
	consumerTestAdd := func(conf string, mirrors []types.Mirror, pluginPrevResult *current.Result, ifName string, hasExternalOwner bool, targetNs ns.NetNS) (string, cnitypes.Result) {
		confMirror, r, err := consumerAdd(version, conf, pluginPrevResult, ifName, targetNs)

		Expect(err).NotTo(HaveOccurred())

		By("Checking mirror ports")
		CheckPortsInMirrors(mirrors, hasExternalOwner, consumerOvsPortOwner, r)
	...
	Context("adding host port to a mirror", func() {
		Context("as consumer (output_port in ovsdb)", func() {
			...
			conf := fmt.Sprintf(`{
				...
				"bridge": "%s",
				"mirrors": %s
			}`, version, consumerBridgeName, mirrorsJSONStr)
			...
			It("should successfully complete ADD, CHECK and DEL commands", func() {
				...
				prevResult := consumerCreateInterfaces(consumerIFNAME1, targetNs)

				By("run ovs-mirror-consumer passing prevResult")
				confMirror, result := consumerTestAdd(conf, mirrors, prevResult, consumerIFNAME1, false, targetNs)
				consumerTestCheck(confMirror, result, consumerIFNAME1, targetNs)
				consumerTestDel(confMirror, mirrors, result, consumerIFNAME1, targetNs)
			})
		})
	})
	...
	Context("adding multiple ports to a single mirror", func() {
		Context("as consumer (output_port in ovsdb)", func() {
			...
			It("shouldn't complete ADD, because you cannot override the configuration of an existing mirror", func() {
				...
				prevResult1 := consumerCreateInterfaces(consumerIFNAME1, targetNs)
				prevResult2 := consumerCreateInterfaces(consumerIFNAME2, targetNs)

				By("run ovs-mirror-consumer ADD command for the first port")
				_, _ = consumerTestAdd(conf, mirrors, prevResult1, consumerIFNAME1, false, targetNs)

				By("run ovs-mirror-consumer ADD command for the second port expecting an error")
				_, _, err := consumerAdd(version, conf, prevResult2, consumerIFNAME2, targetNs)
	...
	Context("adding a mirror with both producer and consumer configuration", func() {
		Context("('output_port', 'select_src_port' and 'select_dst_port' defined with valid portUUIDs)", func() {
			...
			It("shouldn't be removed after calling cmdDel by this plugin, because it contains 'select_src_port' and 'select_dst_port' configured by a producer", func() {
				...
				r2 := consumerCreateInterfaces(consumerIFNAME2, targetNs)
		...
		Context("that are owned by ovs-cni,", func() {
			Context("creating a new mirror", func() {
				...
				It("should remove those that are in the current bridge", func() {
					...
					CreateEmptyMirrors(consumerBridgeName, emptyMirrors, consumerOvsPortOwner)

					By("create interfaces using ovs-cni plugin")
					prevResult := consumerCreateInterfaces(consumerIFNAME1, targetNs)

					By("run ovs-mirror-consumer passing prevResult")
					confMirror, result := consumerTestAdd(conf, mirrors, prevResult, consumerIFNAME1, true, targetNs)
		...
		Context("that are NOT owned by ovs-cni,", func() {
			Context("creating a new mirror", func() {
				...
				It("should NOT remove those that are in the current bridge", func() {
					...
					CreateEmptyMirrors(consumerBridgeName, emptyMirrors, "")
...
// consumerAdd calls cmdAdd with the right input for the mirror-consumer plugin.
func consumerAdd(version string, conf string, pluginPrevResult *current.Result, ifName string, targetNs ns.NetNS) (string, cnitypes.Result, error) {
	...
	r, _, err := cmdAddWithArgs(argsMirror, func() error {
		return consumer.CmdAdd(argsMirror)
	})
